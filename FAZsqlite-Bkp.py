...
def top_http_url(conn, devtype, logtype):
    ...
    if WhiteListURLs:
        with open(white_list_file, 'w') as file:
            file.write('\n'.join(WhiteListURLs))
        logger.info(f"White list updated with {new_urls_count} URLs")
        
        
        # if not os.path.exists(cleaned):
            # logger.info(f"First time cleaning SQLite DB from white listed URLs ...")
            # delete_urls_from_db(conn, logtype, WhiteListURLs)
            # with open(cleaned, 'w') as file:
                # file.write(f"{devtype}.db cleaned")
        
    if new_urls:
        logger.info(f"Start cleaning SQLite DB from white listed URLs ...")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if "database is locked" in str(e) and attempt < max_retries - 1:
                        ...
                        logger.warning(f"Database locked, retrying in {delay:.2f} seconds... (Attempt {attempt + 1}/{max_retries})")
                        time.sleep(delay)
                    else:
                        raise
            logger.error(f"Failed to execute operation after {max_retries} attempts")
...
def delete_urls_from_db(conn, logtype, wl_url):
    try:
        ...
        logger.error(f"SQLite error while deleting URLs: {str(e)}")
        raise   
...
def SQLiteWrite(results_resp_json, conn, logtype, fields):
    ...
    if not data:
        logger.info(f"No data returned from the provided query: {args.query}")
        return
    ...
    try:
            ...
            if args.adom == 'waf':
                ...
                if http_url:
                    if any(pattern.match(http_url) for pattern in exclude_patterns):
                        logger.debug(f"Skipping line due to URL matches exclude regex : {http_url}")
                        continue
                    if http_url in WhiteListURLs:
                        logger.debug(f"Skipping line due to whitelist URL: {http_url}")
                        continue
                ...
                if dstip in WhiteListIPs:
                        logger.debug(f"Skipping line due to whitelist IP: {dstip}")
                        continue
        ...
        conn.rollback()
        logger.error(f"SQLite error in SQLiteWrite: {str(e)}")
        raise
    finally:
        cursor.close()

    logger.debug(f"{len(data)} records has been successfully written to the SQLite database.")
...
def SQLiteRead(conn, devtype, logtype, condition, st=None, et=None):
    ...
    try:
        ...
        logger.error(f"SQLite error in query {query}: {str(e)}")
        return None
...
def build_query(logtype, condition, starttime=None, endtime=None):
    ...
    if condition == '':
        ...
        logger.debug(f"'{base_query} {where_clause} LIMIT 100'")
